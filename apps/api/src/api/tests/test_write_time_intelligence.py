...
class WriteTimeIntelligenceTester:
            ...
            for i, message in enumerate(messages):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        print(
                            f"    ✅ '{message[:30]}...' -> {classification} (actions: {decision})"
                        )
                    ...
                    print(f"    ❌ Exception: {e}")
                    type_results.append({"message": message, "error": str(e)})
    ...
    async def test_batch_processing(self) -> Dict[str, Any]:
        ...
        try:
            response = await self.client.post(
                f"{self.base_url}/write-time/test/batch", json=batch_messages
            )
                ...
                for item in result["results"]:
                    if "error" not in item:
                        ...
                        if (
                            ...
                            == item["content_preview"].split()[0].lower()
                        ):
    ...
    async def test_rate_limiting(self) -> Dict[str, Any]:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    rate_limit_test_results.append(
                        {"message_id": i, "actions": actions, "success": True}
                    )
                else:
                    rate_limit_test_results.append(
                        {
        ...
        print(
            f"   Rate limiting appears to be: {'Active' if successful_requests < 60 else 'Inactive'}"
        )
    ...
    async def test_discard_functionality(self) -> Dict[str, Any]:
        ...
        for message in noise_messages:
            try:
                ...
                discard_results.append({"message": message, "error": str(e)})
    ...
    async def run_full_test_suite(self) -> Dict[str, Any]:
        ...
        if "discard_accuracy" in discard_results:
            print(
                f"  NOISE DISCARD ACCURACY: {discard_results['discarded_messages']}/{discard_results['total_noise_messages']} ({discard_results['discard_accuracy']:.1f}%)"
            )
        ...
        if "rate_limiting_active" in rate_limiting:
            print(
                f"  RATE LIMITING: {'Active' if rate_limiting['rate_limiting_active'] else 'Inactive'}"
            )

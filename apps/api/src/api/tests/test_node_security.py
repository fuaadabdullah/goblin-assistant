...
async def test_client_classifies_503_as_capacity_not_health_failure(monkeypatch):
    ...
    from api.nodes.client import NodeSaturatedError
    ...
    monkeypatch.setattr(node_client.httpx, "AsyncClient", lambda **kwargs: FakeClient())

    with pytest.raises(NodeSaturatedError):
        await node_client.invoke_node(
            endpoint="https://node-001.tailnet:8090",
            model="llama3.1:8b",
            prompt="hi",
        )

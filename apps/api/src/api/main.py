...
try:
    ...
    # Resolve repo root from apps/api/src/api/main.py location.
    # Expected parent chain: api -> src -> api(app root) -> apps -> repo root.
    current_dir = pathlib.Path(__file__).resolve()
    repo_root = current_dir.parents[4]

    # Try to load .env files from the repository root.

...
export default function GoblinDemo({ provider, model, demoMode = false }: Props) {
    const [codeInput, setCodeInput] = useState<string>("// Write code or paste here\nfunction add(a, b) {\n  return a + b;\n}");
    const [orchestration, setOrchestration] = useState<string>("docs-writer: document this code THEN code-writer: write a unit test");
    ...
    const [stepChunks, setStepChunks] = useState<Record<string, Array<{chunk: string, token: number, cost: number}>>>({});
    ...
    const previewOrchestration = async () => {
        console.log("🔍 [DEBUG] Previewing orchestration:", { orchestration, codeInput: codeInput.substring(0, 100) + "..." });
        debugger; // Breakpoint for orchestration preview

        try {
            const client = getRuntimeClient();
            const plan = await client.parseOrchestration(orchestration, codeInput);
            console.log("✅ [DEBUG] Orchestration parsed successfully:", { steps: plan.steps.length, totalBatches: plan.total_batches });
            setPreviewPlan(plan);
    ...
    const handleTemplateChange = (templateName: string) => {
        ...
        debugger; // Breakpoint for template changes
    ...
    const run = async () => {
        console.log("🚀 [DEBUG] Starting execution:", { orchestration, codeLength: codeInput.length, demoMode });
        debugger; // Breakpoint at start of execution
        ...
        try {
            ...
            console.log("✅ [DEBUG] Orchestration parsed:", { steps: parsed.steps?.length || 0, totalBatches: parsed.total_batches });
            setPlan(parsed);
            ...
            for (const step of parsed.steps) {
                console.log("🎬 [DEBUG] Executing step:", { id: step.id, goblin: step.goblin, task: step.task.substring(0, 50) + "..." });
                debugger; // Breakpoint for each step execution
                ...
                try {
                    ...
                    await client.executeTaskStreaming(
                        ...
                        step.task,
                        (chunk: any) => {
                            console.log("📦 [DEBUG] Received chunk:", { stepId: step.id, chunkPreview: (chunk.chunk || JSON.stringify(chunk)).substring(0, 50) + "...", tokenCount: chunk.token_count, costDelta: chunk.cost_delta });
                            ...
                            const chunkText = chunk.chunk || JSON.stringify(chunk);
                            setStreamingText((s: string) => s + chunkText + "\n");
                            const tokenCount = chunk.token_count || chunk.tokenCount || 0;
                            const costDelta = chunk.cost_delta || chunk.costDelta || 0;
                            if (tokenCount) {
                        ...
                        (final: any) => {
                            console.log("🏁 [DEBUG] Step completed:", { stepId: step.id, cost: final?.cost, reasoning: final?.reasoning?.substring(0, 50) + "..." });
                            setStreamingText((s: string) => s + `--- Step ${step.id} COMPLETE ---\n` + JSON.stringify(final) + "\n");
                            const c = final?.cost || 0;
                    ...
                    setStreamingText((s: string) => s + `--- Step ${step.id} FAILED ---\nError: ${stepError?.message || String(stepError)}\n`);
            ...
            setStreamingText((s: string) => s + `Error: ${err?.message || String(err)}\n`);
    ...
    const fallbackToNonStreaming = async () => {
        try {
            ...
            setStreamingText((s: string) => s + `Fallback also failed: ${fallbackError?.message || String(fallbackError)}\n`);
        }
    };
    ...
    return (
        <div className="goblin-demo" data-testid="goblin-demo">
            <div className="inputs" data-testid="goblin-inputs">
                <label htmlFor="codeInput" data-testid="code-input-label">Code</label>
                <textarea
                    id="codeInput"
                    value={codeInput}
                ...
                <label data-testid="template-select-label">Orchestration Template</label>
                <select
                ...
                >
                    {ORCHESTRATION_TEMPLATES.map(template => (
                        <option key={template.name} value={template.name} data-testid={`template-option-${template.name.replace(/\s+/g, '-').toLowerCase()}`}>
                            {template.name} - {template.description}
                        </option>
                    ))}
                </select>

                <label htmlFor="orch" data-testid="orchestration-label">Orchestration</label>
                <input
                    id="orch"
                    value={orchestration}
                ...
                {previewPlan && (
                    <div className="plan-preview" data-testid="plan-preview">
                        <h4 data-testid="plan-preview-title">Plan Preview ({previewPlan.steps.length} steps)</h4>
                        <div className="estimated-cost" data-testid="estimated-cost">
                        ...
                        <ul className="plan-steps" data-testid="plan-steps">
                            {previewPlan.steps.map((step, index) => (
                                <li key={step.id} className="plan-step" data-testid={`plan-step-${step.id}`}>
                                    <span className="step-number" data-testid={`step-number-${step.id}`}>{index + 1}.</span>
                                    <span className="step-goblin" data-testid={`step-goblin-${step.id}`}>{step.goblin}:</span>
                                    <span className="step-task" data-testid={`step-task-${step.id}`}>{step.task}</span>
                                </li>
                            ))}
                        </ul>
                    </div>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 12 }} data-testid="goblin-results">
                {/* Plan preview */}
                {Object.keys(stepCosts).length > 0 && (
                    <div className="plan-total" data-testid="plan-total">
                        <strong>Total Plan Cost: </strong>{`$${(Object.values(stepCosts) as number[]).reduce((a, b) => a + b, 0).toFixed(6)}`}
                    </div>
                )}
                {plan && (
                    <div className="plan-preview" data-testid="execution-plan">
                        ...
                        <ol data-testid="execution-steps">
                            {plan.steps.map((s: OrchestrationStep) => (
                                                ...
                                                >
                                                    <strong data-testid={`execution-step-goblin-${s.id}`}>{s.goblin}</strong>: <span data-testid={`execution-step-task-${s.id}`}>{s.task}</span>
                                                    {' '}
                                                    <em data-testid={`execution-step-status-${s.id}`}>({stepStatuses[s.id] || 'pending'})</em>
                                                    {' '}
                                                    <span style={{ marginLeft: 12 }} data-testid={`execution-step-cost-${s.id}`}>{(stepCosts[s.id] || 0) > 0 ? `$${(stepCosts[s.id] || 0).toFixed(6)}` : ''}</span>
                                                    {expandedSteps[s.id] && (
                                                        <div className="step-details" data-testid={`step-details-${s.id}`}>
                                                            ...
                                                            <div data-testid={`step-cost-${s.id}`}>Cost: ${((stepCosts[s.id] || 0)).toFixed(6)}</div>
                                                                    ...
                                                                    <ul data-testid={`chunks-${s.id}`}>
                                                                        {stepChunks[s.id].map((c, idx) => (
                                                                            <li key={`chunk-${s.id}-${idx}`} data-testid={`chunk-${s.id}-${idx}`}>
                                                                                <span className="chunk-text" data-testid={`chunk-text-${s.id}-${idx}`}>{c.chunk}</span>
                                                                                <span className="chunk-meta" data-testid={`chunk-tokens-${s.id}-${idx}`}>Tokens: {c.token}</span>
                                                                                <span className="chunk-meta" data-testid={`chunk-cost-${s.id}-${idx}`}>Cost: ${c.cost.toFixed(6)}</span>
                                                                                <div className="chunk-graph" style={{ ['--w' as any]: `${Math.min(100, c.cost * 1000)}%` }} data-testid={`chunk-graph-${s.id}-${idx}`} />
                                                                            </li>
                                                                        ))}
                                                                    </ul>
                                                                </div>

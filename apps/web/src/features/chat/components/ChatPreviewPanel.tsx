...
const ChatPreviewPanel = () => {
  ...
  const samplePrompt = "Hey Goblin, can you summarize last quarter's revenue and flag anything surprising?";
  const loginWithPrompt = (prompt: string) => ({ pathname: '/login', query: { from: `/chat?prompt=${encodeURIComponent(prompt)}` } });
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          <div className="max-w-[80%] text-right ml-auto">
            <Link href={loginWithPrompt(samplePrompt)} aria-label="Sign in to continue this conversation">
      ...
      <div className="mt-auto pt-6">
        <div className="text-sm text-muted">This preview is static — sign in to continue the conversation.</div>
        <div className="mt-3">
          <Input placeholder="Sign in to continue this conversation..." disabled />
        </div>
        <div className="mt-3 grid grid-cols-1 gap-2">
          <Link href={loginHref} className="inline-flex items-center justify-center px-4 py-2 rounded-lg bg-primary text-text-inverse font-medium">
            Sign in to Goblin →
          </Link>
          <Link href={{ pathname: '/login', query: { mode: 'register' } }} className="inline-flex items-center justify-center px-4 py-2 rounded-lg border border-primary text-primary font-medium">
            Create account
          </Link>
        </div>
      </div>

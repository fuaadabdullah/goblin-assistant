...
class MetricsCollector:
    ...
    async def collect_system_metrics(
        self, user_id: Optional[str] = None, time_window_hours: int = 24
    ) -> SystemMetrics:
    ...
    async def _generate_recommendations(
        ...
        if memory_health.get("status") == "critical":
            recommendations.append(
                "Memory health is critical - review promotion criteria and gate thresholds"
            )
        elif memory_health.get("status") == "warning":
            if memory_health.get("promotion_rate", 0) < 10:
                recommendations.append(
                    "Low memory promotion rate - consider reviewing promotion criteria"
                )
        ...
        if retrieval_quality.get("status") == "critical":
            recommendations.append(
                "Retrieval quality is critical - review embedding models and retrieval algorithms"
            )
        elif retrieval_quality.get("status") == "warning":
            if retrieval_quality.get("avg_relevance", 0) < 0.6:
                recommendations.append(
                    "Low retrieval relevance - review semantic search parameters"
                )
        ...
        if context_assembly.get("status") == "critical":
            recommendations.append(
                "Context assembly is critical - review layer processing and token allocation"
            )
        elif context_assembly.get("status") == "warning":
            if context_assembly.get("avg_assembly_time", 0) > 500:
                recommendations.append(
                    "Slow context assembly - optimize layer processing performance"
                )

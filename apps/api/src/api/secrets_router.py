...
async def init_secrets_adapter() -> None:
    ...
    try:
        ...
        if secrets_backend == "env":
            # Environment-variable backend – works out of the box,
            # no external vault required.
            _secrets_adapter = create_adapter("env")
            logger.info(
                "Initialized env-var secrets adapter (no external vault configured). "
                "Set SECRETS_BACKEND=vault with VAULT_TOKEN or VAULT_ROLE_ID+VAULT_SECRET_ID "

...
export async function GET() {
  try {
    const response = await fetchWithTimeout(`${BACKEND_URL}/api/v1/health`, 5000);
    ...
    const reason = error instanceof Error && error.name === 'AbortError' ? 'backend_timeout' : 'backend_unreachable';

...
test.describe('Brutal release gate', () => {
  ...
  test('huge conversations load, stay readable, and keep accepting new turns', async ({
    page,
    context,
  }) => {
    const harness = await prepareAuthedPage(context, page);
    const hugeConversationId = 'conv-huge';
    const hugeMessages = Array.from({ length: 160 }, (_, index) => {
      const turnNumber = index + 1;
      return {
        message_id: `msg-${String(turnNumber).padStart(4, '0')}`,
        role: turnNumber % 2 === 0 ? ('assistant' as const) : ('user' as const),
        content: `History line ${turnNumber}`,
        timestamp: '2026-08-18T12:00:00.000Z',
      };
    });
    harness.state.addConversation(hugeConversationId, 'Huge conversation', hugeMessages);

    await openAuthedPage(page, '/chat');
    await waitForChat(page);
    const hugeConversationButton = page.getByRole('button', { name: /Huge conversation/i });
    await expect(hugeConversationButton).toBeVisible();
    await hugeConversationButton.scrollIntoViewIfNeeded();
    await hugeConversationButton.dispatchEvent('click');

    await expect(chatTranscript(page)).toContainText('History line 1');
    await expect(chatTranscript(page)).toContainText('History line 160');

    await page.getByLabel(/chat message input/i).fill('Huge conversation check.');
    await page.getByRole('button', { name: /send message/i }).click();

    await expect(chatTranscript(page)).toContainText('Huge conversation loaded and preserved.');
    await page.reload({ waitUntil: 'domcontentloaded' });
    await waitForChat(page);
    await expect(chatTranscript(page)).toContainText('Huge conversation loaded and preserved.');
  });

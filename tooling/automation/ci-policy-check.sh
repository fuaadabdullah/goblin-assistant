...
# matter what the author actually wrote. CircleCI checks out the branch head
# instead, which is why the identical script passed there and failed here.
#
# Excluding merges is correct in its own right too: merge subjects are
# generated by git and GitHub rather than authored, so holding them to an
# authoring convention is meaningless.
#

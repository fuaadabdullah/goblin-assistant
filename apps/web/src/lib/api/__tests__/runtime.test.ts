...
const mockPostBackend = postBackend as jest.MockedFunction<typeof postBackend>;
...
describe('apiClient runtime methods', () => {
  it('getGoblins calls /api/goblins', async () => {
    ...
    expect(mockGetBackend).toHaveBeenCalledWith('/api/goblins');
  });
  ...
  it('getHistory calls /api/history/{goblin} with capped limit', async () => {
    ...
    expect(mockGetBackend).toHaveBeenCalledWith('/api/history/docs-writer?limit=100');
  });
  ...
  it('getStats calls /api/stats/{goblin}', async () => {
    ...
    expect(mockGetBackend).toHaveBeenCalledWith('/api/stats/demo');
  });
  ...
  it('parseOrchestration posts /api/orchestrate/parse', async () => {
    ...
    expect(mockPostBackend).toHaveBeenCalledWith('/api/orchestrate/parse', {
      text: 'a THEN b',
      default_goblin: 'demo',
    });
  });

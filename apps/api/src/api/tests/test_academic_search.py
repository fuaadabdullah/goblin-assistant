...
async def test_arxiv_search_success():
    ...
    with patch(
        "api.assistant_tools.skills.academic_search.get_text",
        return_value=_ARXIV_ATOM_XML,
    ):
...
async def test_arxiv_category_filter_sent_in_query():
    ...
    mock_get = AsyncMock(return_value="<feed xmlns='http://www.w3.org/2005/Atom'></feed>")
    with patch(
        "api.assistant_tools.skills.academic_search.get_text",
        new=mock_get,
    ):
...
async def test_semantic_scholar_search_success():
    ...
    with patch(
        "api.assistant_tools.skills.academic_search.get_json",
        return_value=_SEMANTIC_SCHOLAR_JSON,
    ):
        from api.assistant_tools.skills.academic_search import _handle_academic_search

...
- `MINIO_ROOT_USER`: MinIO root user for local object storage
- `MINIO_ROOT_PASSWORD`: MinIO root password for local object storage
- `S3_ACCESS_KEY`: S3-compatible access key used by sandbox worker
- `S3_SECRET_KEY`: S3-compatible secret key used by sandbox worker
- `FLOWER_BASIC_AUTH`: Basic auth credentials for local Flower dashboard (format: `user:password`)

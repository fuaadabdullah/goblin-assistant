...
def test_heartbeat_accepts_the_agent_payload(client):
    resp = client.post("/nodes/heartbeat", json=HEARTBEAT)
...
def test_second_heartbeat_is_a_refresh_not_a_registration(client):
    client.post("/nodes/heartbeat", json=HEARTBEAT)
    body = client.post("/nodes/heartbeat", json=HEARTBEAT).json()
...
def test_heartbeat_rejects_a_payload_with_no_node_id(client):
    ...
    assert client.post("/nodes/heartbeat", json=bad).status_code == 422


def test_heartbeat_rejects_an_unknown_status(client):
    assert client.post(
        "/nodes/heartbeat", json={**HEARTBEAT, "status": "vibing"}
    ).status_code == 422


def test_heartbeat_rejects_negative_active_jobs(client):
    assert client.post(
        "/nodes/heartbeat", json={**HEARTBEAT, "active_jobs": -1}
    ).status_code == 422
...
def test_saturated_node_is_listed_but_not_eligible(client):
    client.post("/nodes/heartbeat", json={**HEARTBEAT, "active_jobs": 1})
    node = client.get("/nodes").json()[0]
...
def test_agent_payload_shape_is_accepted_verbatim(client):
    ...
    assert client.post("/nodes/heartbeat", json=agent_payload).status_code == 200

...
| `cd apps/api && PYTHONPATH=src python3.11 -m pytest -o "addopts=" -q src/api/tests/test_provider_health_service.py::test_validate_configured_credentials_groups_ids src/api/tests/test_provider_health_service.py::test_validate_configured_credentials_keeps_startup_contract_keys` | Pass | Restores startup compatibility keys for provider credential diagnostics. A follow-up full `make test-api` was interrupted by SIGTERM at 93%, so the completed full-suite count remains the 13-failure run above. |
...
| Provider health contract drift | Credential validation tests no longer received the expected `invalid_credentials` and `unreachable` keys. | Focused provider-health tests now pass after restoring startup compatibility keys. Needs a completed full-suite rerun before removing it from release-risk accounting. |

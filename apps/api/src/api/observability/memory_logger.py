...
class MemoryPromotionLogger:
    ...
    def _log_promotion_structured(self, event: MemoryPromotionEvent):
        ...
        log_data = {
            ...
            "promotion_event": {
                ...
                "candidate_preview": (
                    event.candidate_text[:100] + "..."
                    if len(event.candidate_text) > 100
                    else event.candidate_text
                ),
                "source": {
        ...
        if event.promotion_decision:
            logger.info("PROMOTION: Memory fact promoted", extra={"promotion": log_data})
        else:
            logger.warning("PROMOTION: Memory fact rejected", extra={"promotion": log_data})
    ...
    async def get_promotion_stats(
        ...
        return {
            ...
            "avg_confidence": {
                "promoted": round(
                    sum(e.confidence_score for e in relevant_events if e.promotion_decision)
                    / max(1, promoted),
                    2,
                ),
                "rejected": round(
                    sum(e.confidence_score for e in relevant_events if not e.promotion_decision)
                    / max(1, rejected),
                    2,
                ),
            },
    ...
    async def get_memory_health_report(self, user_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    def _generate_health_recommendations(
        ...
        if contradiction_rate > 5:
            recommendations.append(
                "High contradiction rate detected - review memory promotion criteria"
            )
        ...
        if gate_failures.get("content_quality", 0) > gate_failures.get("repetition", 0):
            recommendations.append(
                "Content quality gate failing frequently - review content quality filters"
            )
    ...
    async def search_promotions(
        ...
        for event in self._promotion_cache.values():
            ...
            if query:
                event_text = (
                    f"{event.candidate_text} {event.category} {event.rejection_reason or ''}"
                )

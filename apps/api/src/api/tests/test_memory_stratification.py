...
class TestMemoryPromotion:
    ...
    def test_fact_extraction_from_preference_message(self):
        ...
        facts = self.promotion_service._extract_facts_from_preference_message(
            content, classification
        )
...
class TestEndToEndPipeline:
    ...
    async def test_complete_pipeline(self):
        ...
        conversation = await conversation_store.create_conversation(
            user_id="test_user_123", title="Test Memory Pipeline"
        )
        ...
        promotion_result = await memory_promotion_service.promote_from_conversation(
            conversation.conversation_id, "test_user_123"
        )
        ...
        context = await retrieval_service.retrieve_context(
            query="Tell me about the user's preferences", user_id="test_user_123", k=5
        )
...
class TestPerformance:
    ...
    def test_classification_performance(self):
        ...
        repeated_times = timeit.repeat(run_classifications, repeat=3, number=1)
        total_time = min(repeated_times)

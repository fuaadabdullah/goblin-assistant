...
class ContextMonitoringService:
        ...
        try:
            ...
            if "assembly_log" in assembly_result:
                ...
                start_time = datetime.fromisoformat(
                    assembly_log.get("assembly_time", datetime.utcnow().isoformat())
                )
                ...
                for layer_name in assembly_log.get("layers", []):
                    ...
                    layers.append({"name": layer_name, "tokens": layer_tokens, "success": True})
    ...
    def get_assembly_performance(self) -> Dict[str, Any]:
        ...
        avg_assembly_time = (
            sum(self.assembly_times) / len(self.assembly_times) if self.assembly_times else 0.0
        )
    ...
    def get_debug_info(self) -> Dict[str, Any]:
        ...
        return {
            "assembly_service": (
                self.assembly_service.get_debug_info() if self.assembly_service else {}
            ),
            "system_prompt": (
                self.system_prompt_manager.get_debug_info() if self.system_prompt_manager else {}
            ),

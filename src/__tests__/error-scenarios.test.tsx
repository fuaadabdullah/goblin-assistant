...
describe('Error Scenarios - Model Fetch Failures', () => {
  ...
  it('should handle provider API unavailability gracefully', async () => {
    const { runtimeClient } = await import('../api/tauri-client')
    vi.mocked(runtimeClient).getProviderModels.mockRejectedValue(new Error('Network error'))

    const { container } = render(<TestWrapper><ProviderSelector providers={[]} onChange={vi.fn()} /></TestWrapper>)
  ...
  it('should handle malformed API responses', async () => {
    const { runtimeClient } = await import('../api/tauri-client')
    vi.mocked(runtimeClient).getProviderModels.mockResolvedValue('invalid response' as any)
...
describe('Error Scenarios - Orchestration Errors', () => {
  ...
  it('should handle command execution failures', async () => {
    ...
    render(<TestWrapper><GoblinDemo /></TestWrapper>)
    ...
    await waitFor(() => {
      const streamingOutput = screen.getByTestId('streaming-container').querySelector('.streaming-output')
      expect(streamingOutput?.textContent).toContain('client.executeTaskStreaming is not a function')
    }, { timeout: 3000 })
  })
  ...
  it('should handle missing orchestration dependencies', async () => {
    ...
    vi.mocked(runtimeClient).executeGoblinCommand.mockRejectedValue(new Error('Missing dependency: tool not found'))
...
describe('Error Scenarios - Component Integration Failures', () => {
  ...
  it('should handle invalid provider selection', async () => {
    ...
    render(<TestWrapper><ModelSelector provider="invalid" onChange={vi.fn()} /></TestWrapper>)
  ...
  it('should handle null/undefined props gracefully', async () => {
    render(<TestWrapper><ModelSelector provider={undefined as any} onChange={vi.fn()} /></TestWrapper>)
  ...
  it('should handle rapid component unmounting', async () => {
    ...
    const { unmount } = render(<TestWrapper><ProviderSelector providers={[]} onChange={vi.fn()} /></TestWrapper>)

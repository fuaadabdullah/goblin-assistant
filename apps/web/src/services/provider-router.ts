...
function isJsonProviderEntry(value: unknown): value is JsonProviderEntry {
  if (!isRecord(value)) return false;

  return (
    isOptionalString(value['endpoint']) &&
    isOptionalString(value['endpoint_env']) &&
    isOptionalString(value['endpoint_fallback']) &&
    isOptionalString(value['api_key_env']) &&
    (value['priority_tier'] === undefined || isNumber(value['priority_tier'])) &&
    (value['capabilities'] === undefined || isStringArray(value['capabilities'])) &&
    (value['models'] === undefined || isStringArray(value['models'])) &&
    (value['cost_score'] === undefined || isNumber(value['cost_score'])) &&
    (value['default_timeout_ms'] === undefined || isNumber(value['default_timeout_ms'])) &&
    (value['rate_limit_per_min'] === undefined || isNumber(value['rate_limit_per_min'])) &&
    (value['display_name'] === undefined || typeof value['display_name'] === 'string') &&
    (value['is_active'] === undefined || typeof value['is_active'] === 'boolean') &&
    isOptionalString(value['invoke_path'])
  );
}
...
try {
  ...
  Object.entries(config.providers || {}).forEach(([id, provider]) => {
    if (!isJsonProviderEntry(provider)) {
      devWarn(`Provider schema mismatch: ${id} has an invalid entry shape`);
      return;
    }
...
function scoreProvider(
  ...
  const candidate = PROVIDERS[pid];
  if (!isJsonProviderEntry(candidate)) return Infinity;
  const p = candidate;

...
describe("GoblinDemo", () => {
	beforeEach(() => {
		...
		mockParseOrchestration.mockResolvedValue({
			steps: [
				...
				{ id: "step2", goblin: "code-writer", task: "write a unit test", dependencies: ["step1"], batch: 1 },
			],
			total_batches: 1,
			max_parallel: 1,
		});
	...
	it("displays default code input", () => {
		...
		expect(codeInput).toHaveValue("// Write code or paste here\nfunction add(a, b) {\n  return a + b;\n}");
	});
	...
	it("displays default orchestration input", () => {
		...
		expect(orchInput).toHaveValue("docs-writer: document this code THEN code-writer: write a unit test");
	});
	...
	it("changes orchestration when template is selected", () => {
		...
		const orchInput = screen.getByTestId("orchestration-input");
		expect(orchInput).toHaveValue("code-writer: analyze this code for issues THEN code-writer: suggest optimizations");
	});
	...
	it("disables run button while running", async () => {
		...
		mockExecuteTaskStreaming.mockImplementation(() => new Promise(resolve => setTimeout(resolve, 100)));
	...
	it("shows step status and cost information", async () => {
		...
		// Check initial status (should be running since execution starts immediately)
		expect(screen.getByTestId("execution-step-status-step1")).toHaveTextContent("(running)");
	...
	it("handles parsing errors gracefully", async () => {
		...
		await waitFor(() => {
			expect(consoleSpy).toHaveBeenCalledWith("❌ [DEBUG] Failed to preview orchestration:", expect.any(Error));
		});

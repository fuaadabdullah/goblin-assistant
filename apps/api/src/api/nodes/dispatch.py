...
async def try_local_compute(
    ...
    *,
    ...
    if not reg.try_reserve(node.node_id, model=wanted):
        logger.debug(
            "local_compute_skipped",
    ...
    try:
        ...
        except NodeSaturatedError as exc:
            # Healthy-but-busy is flow control, not a circuit-breaker failure.
            logger.info(

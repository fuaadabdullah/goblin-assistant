...
const AVAILABLE_GOBLINS = [
  { id: 'docs-writer', name: 'Documentation Writer', description: 'Documents code and writes comments' },
...
const CONDITION_TYPES = [
  { value: 'THEN', label: 'Then (Sequential)', description: 'Execute next step after this completes' },
  { value: 'AND', label: 'And (Parallel)', description: 'Execute simultaneously with previous step' },
  { value: 'IF_SUCCESS', label: 'If Success', description: 'Execute only if previous step succeeds' },
  { value: 'IF_FAILURE', label: 'If Failure', description: 'Execute only if previous step fails' }
];

export default function WorkflowBuilder({ onOrchestrationChange, initialOrchestration = '' }: Props) {
      ...
      {isBuilderMode ? (
        <div className="visual-builder">
          <div className="step-adder">
            ...
            <button onClick={addStep} className="add-step-btn" disabled={!selectedGoblin || !taskInput.trim()}>
              Add Step
            </button>
          </div>
                ...
                <div className="step-content">
                  {index > 0 && (
                      <select
                        ...
                        onChange={(e) => updateStepCondition(step.id, e.target.value as WorkflowStep['condition'])}
                  ...
                  <div className="step-details">
                    <strong>{AVAILABLE_GOBLINS.find(g => g.id === step.goblin)?.name || step.goblin}</strong>
        ...
        <div className="text-mode">
          <p>Switch to Visual Builder to create workflows step-by-step, or edit the orchestration text directly.</p>
        </div>
      )}

      <div className="orchestration-output">
        <label>Generated Orchestration:</label>
        <code className="orchestration-text">{buildOrchestrationFromSteps() || 'No steps configured'}</code>
      </div>
    </div>
  );
}

...
| --- | --- | --- | --- | --- |
| Auth: login/register/logout/validate | `apps/web/src/lib/api/auth.ts` | `/api/v1/auth/login`, `/api/v1/auth/register`, `/api/v1/auth/logout`, `/api/v1/auth/validate` | Implemented | The client calls same-origin `/api/auth/*` only; `assertNoVersionedClientPath` rejects `/v1` paths from the browser. `google/url`, `google/callback` and `validate` have explicit route handlers; the rest reach the backend through the `[...path]` proxy. |
...
| Account | `apps/web/src/lib/api/account.ts` | `/api/v1/account/profile`, `/api/v1/account/preferences` | Implemented | The client uses the internal `/api/account` prefix; both save paths reach the backend through the proxy, not directly. |

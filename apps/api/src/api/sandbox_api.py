...
from .core.source_code_validation import validate_source_code
...
# Per-user concurrency limit (from our branch)
MAX_CONCURRENT_PER_USER = int(os.getenv("SANDBOX_MAX_PER_USER", "5"))
# Output size cap when reading logs (from our branch)
...
async def submit_job(
    ...
    # Validate source code for dangerous patterns (from our branch)
    req.source, _code_validation = validate_source_code(req.source, req.language)

    # Per-user concurrency limit (from our branch)
    if x_api_key:
        user_key = f"sandbox:user:{x_api_key}:active_jobs"
        active_jobs = int(await _run_blocking(r.get, user_key) or 0)
        if active_jobs >= MAX_CONCURRENT_PER_USER:
            raise HTTPException(
                status_code=429,
                detail=(
                    f"Too many active sandbox jobs ({active_jobs}). "
                    f"Maximum {MAX_CONCURRENT_PER_USER} concurrent jobs per user."
                ),
            )
    ...
    # Track per-user active job count (from our branch)
    if x_api_key:
        user_key = f"sandbox:user:{x_api_key}:active_jobs"
        await _run_blocking(r.incr, user_key)
        await _run_blocking(r.expire, user_key, 3600)  # Auto-expire after 1 hour
    ...
    try:
        ...
        # Decrement active job counter on failure
        if x_api_key:
            user_key = f"sandbox:user:{x_api_key}:active_jobs"
            await _run_blocking(r.decr, user_key)
...
async def get_job_logs(
    ...
    try:
        ...
        # Enforce output size cap (from our branch)

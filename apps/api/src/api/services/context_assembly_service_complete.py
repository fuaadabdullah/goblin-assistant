    ...
    async def assemble_context(
        ...
        try:
            ...
            if remaining_tokens > 0:
                long_term_layer = await self._assemble_long_term_memory(user_id, remaining_tokens)
                if long_term_layer:
            ...
            context_snapshot_id = await context_snapshotter.create_snapshot(
                ...
                hard_stops_applied=any(
                    layer.metadata.get("hard_stop_applied", False) for layer in layers
                ),
            )
    ...
    async def _assemble_long_term_memory(
        ...
        try:
            ...
            if tokens > self.budget.long_term_tokens:
                memory_content = self._trim_to_tokens(memory_content, self.budget.long_term_tokens)

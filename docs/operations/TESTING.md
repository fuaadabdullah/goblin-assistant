...
Root strategy visibility now lives under `tests/`:
...
Machine-readable bucket manifests live in `tests/manifests/*.json` and map to current in-place test locations.
Use root entrypoints:
...
| File | Purpose |
|------|---------|
| `apps/web/src/components/auth/ModularLoginForm.test.tsx` | Login/registration form |
| `apps/web/src/components/chat/ChatInterface.test.tsx` | Chat message display & input |
| `apps/web/src/hooks/useAuth.test.ts` | Authentication hook |
| `apps/web/src/services/api.test.ts` | API client |
...
# Run specific test file
npm run test:unit -- apps/web/src/components/auth/ModularLoginForm.test.tsx
...
See `docs/operations/CRITICAL_TESTING_POLICY.md` for Tier 0 test priorities, contract requirements, and E2E budget enforcement.
